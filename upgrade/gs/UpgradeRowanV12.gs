...
(Object
	...
	instVarNames: #(audit repairedCount skipCount repairedByReload errorMessages auditErrors projectsHome skip shouldCommit upgradeFrom)
	classVars: #()
	classInstVars: #()
	poolDictionaries: #()
...
step_2_repairRowanAuditFailures
	...
	(self gemstoneVersion >= '3.7.5' asRwGemStoneVersionNumber
		and: [ self upgradeFrom asRwGemStoneVersionNumber = '3.6.2' asRwGemStoneVersionNumber ])
		ifTrue: [ 
			"new TonelSTONWriter version in upgrade to 3.7.5 (Rowan v1.2.17)from 3.6.2 (Rowan v1.2.14). The oop of the superclass STONWriter changed during upgrade, but NO shape changes to be found"
			Rowan packageTools disown disownClassNamed: 'TonelSTONWriter'.
