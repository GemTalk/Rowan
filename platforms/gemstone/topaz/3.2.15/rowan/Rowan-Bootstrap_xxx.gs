...
%
  ...
  commit
   run
  "Bootstrap Rowan into image -- Kernel class extensions - Installed and managed as SystemUser/Globals extensions"
  | packageManager repo |
  ...
  #( 'Rowan-GemStone-Core' 'Rowan-GemStone-3215')
    do: [ :packageName | 
      packageManager
...
#  login
  ...
  run
	| symbolName session symbolList symbolDict size |
	symbolName := #RowanShadow.
...
%
  ...
  repo := CypressAbstractRepository
    ...
    alias: ''.
  packageManager
    defaultSymbolDictionaryName: #'RowanShadow'.
  #('Cypress-Core' 'GemStone-Interactions' 'Rowan-Url' 'Rowan-Core' 'Rowan-Definitions' 'Rowan-Cypress' 'Rowan-Tools' 'Rowan-Tests' 'Rowan-Services')
    do: [ :packageName | 
      packageManager
        addResolvedReference:
          (CypressResolvedReference name: packageName repository: repo) ].
...
# Install Rowan using Rowan
  run
  | projectSetDefinition gitRepoPath rowanClass rwSpecificationClass |
  rowanClass := Rowan.
  projectSetDefinition := RwProjectSetDefinition new.
  rwSpecificationClass := RwSpecification.
  System myUserProfile removeDictionaryAt: (System myUserProfile symbolList indexOf: RowanShadow).
  ...
  #(
    ...
    [ 
      rowanClass projectTools load
        loadProjectSetDefinition: projectSetDefinition 
        instanceMigrator: rowanClass platform instanceMigrator.
   ...
   true
	...
	select: [:cl | 
		(rowanClass image loadedClassNamed: cl name asString ifAbsent: [])
			handle ~~ cl ].
...
# Install Cypress, STON, and Tonel using Rowan
  run
